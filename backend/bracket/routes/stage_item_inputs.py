...
async def validate_stage_item_update(
    stage_item_input: StageItemInput | None, tournament_id: TournamentId
) -> None:
    ...
    if isinstance(stage_item_input, StageItemInputUpdateBodyTentative):
        input_id = stage_item_input.winner_from_stage_item_id
    ...
    if isinstance(stage_item_input, StageItemInputUpdateBodyFinal):
        if await get_team_by_id(stage_item_input.team_id, tournament_id) is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Could not find team with id {stage_item_input.team_id}",
            )
...
async def update_stage_item_input(
    ...
    await validate_stage_item_update(stage_item_input, tournament_id)
