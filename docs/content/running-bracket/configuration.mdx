...
- `API_PREFIX`: Must be set to `/api` when `SERVE_FRONTEND` is True. This makes all the backend requests go to
  `localhost:8400/api/ping` instead of `localhost:8400/ping`. Make sure that `VITE_API_BASE_URL` of the frontend
  contains this prefix as well.
